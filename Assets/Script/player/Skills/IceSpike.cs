...
public class IceSpike : MonoBehaviour
{
    ...
    [SerializeField] private GameObject IceSpikePrefab; // Reference to the lightning strike prefab.
    ...
    public void ActivateSkill()
    {
        ...
        if (SkillManager.instance.CanUseSkill("Ice"))
        {
            //Spawn on the player.
            // Spawn the Ice Spike on the player.
            CreateIceSpike(player.transform.position);

            // Trigger the cooldown through the SkillManager
            SkillManager.instance.TriggerCooldown("Ice");
        }
    }
    ...
    private void CreateIceSpike(Vector3 position)
    {
        // Instantiate the Ice Spike prefab at the specified position.
        GameObject IceSpike = Instantiate(IceSpikePrefab, position, Quaternion.identity);
